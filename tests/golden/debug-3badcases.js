...
async function runQuestion(dataset, question) {
  ...
  const sample = csv.length > 500000 ? csv.slice(0, 500000) : csv; // large files truncated for speed
  ...
  if (a.rawResult && a.rawResult.length > 0) {
    console.log('first row: ' + JSON.stringify(a.rawResult[0]));
  }
  console.log('');
}
...
async function main() {
  ...
  await runQuestion('sales', '按销售区域统计总销售额，找出最高区域');
  await runQuestion('sales', '按日期统计每日销售额，找出最高和最低日期');
}
