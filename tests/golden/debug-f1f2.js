...
async function runQuestion(dataset, question) {
  ...
  console.log('isScalar: ' + win._isScalar);
  console.log('multiMetricFields: ' + JSON.stringify(win._lastMultiMetricFields));
  console.log('chartType: ' + a.chartType);
  console.log('chartData keys: ' + (a.chartData ? Object.keys(a.chartData).join(',') : 'null'));
  if (a.chartData && a.chartData.series) {
    console.log('chartData.series.length: ' + a.chartData.series.length);
    a.chartData.series.forEach((s, i) => console.log('  series[' + i + ']: name=' + s.name + ', values.length=' + (s.values ? s.values.length : 0)));
  }
  if (a.chartData && a.chartData.categories) {
    console.log('chartData.categories: ' + JSON.stringify(a.chartData.categories));
  }
  console.log('rows.length: ' + (a.rawResult ? a.rawResult.length : 'null'));
  if (a.rawResult && a.rawResult.length > 0) {
    console.log('first 3 rows:');
    a.rawResult.slice(0, 3).forEach((r, i) => console.log('  [' + i + '] ' + JSON.stringify(r)));
  }
  console.log('interpretation: ' + (a.interpretation || '').slice(0, 200));
  console.log('');
}

async function main() {
  await runQuestion('sales', '计算总销售额和总销售数量');
  await runQuestion('sales', '按产品类别统计总销售额和总数量');
}
