...
async function runQuestion(dataset, question) {
  ...
  console.log('sql: ' + (a.sql || '').slice(0, 250).replace(/\n/g, ' '));
  ...
  console.log('_lastFullRanking: ' + (win._lastFullRanking ? JSON.stringify(win._lastFullRanking).slice(0, 200) : 'NULL'));
  console.log('');
}
...
async function main() {
  ...
  for (const q of Q) {
    await runQuestion('sales_team', q);
  }
}
