...
async function run() {
  ...
  const csv = fs.readFileSync(path.join(TEST_DIR, 'sales_team.csv'), 'utf-8');
  win.Papa.parse(csv, {
    ...
    complete: (results) => {
      ...
      win.parsedData = { fileName: 'sales_team.csv', fileSize: csv.length, rows, columns: headers, fieldInfo, typeDistribution: [] };
    }
  });
  await new Promise(r => setTimeout(r, 100));

  const a = await win.analyzeQuestion('张伟的总销售额是多少？');
  console.log('=== Q24 ===');
  console.log('type:', a.type);
  console.log('sql:', a.sql);
  console.log('rows:', JSON.stringify(a.rawResult));
  console.log('interpretation:', a.interpretation);
  console.log('_lastExtraWhere:', JSON.stringify(win._lastExtraWhere));
  console.log('_lastFullRanking:', win._lastFullRanking ? win._lastFullRanking.fullResult.length + ' ent totalSum=' + win._lastFullRanking.totalSum.toFixed(2) : 'NULL');
}
